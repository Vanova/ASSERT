...
def my_config():
    ...
    data_files = {  # training
        ...
        'dev_scp': '/home/vano/wrkdir/projects/antispoofing_speech/kaldi_feats/data/tasks_spec/feats.scp',
...
def prediction(val_loader, model, device, output_file, utt2systemID_file, rnn, focal_obj):
    ...
    with torch.no_grad():
        for i, (utt_list, input, target) in enumerate(val_loader):
            ...
            score = 1. -  1. / (1 + np.exp(-output[:, 1]))  # use log-probability of the bonafide class for scoring
